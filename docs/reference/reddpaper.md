# ReddPaper

**Title:** *REDD PAPER — A look into our technology*  
**Date:** Aug 2019  
**Authors:** Jay “TechAdept” Laurence & John “Gnasher” Nash  

PDF: [Redd-Paper.pdf](../assets/docs/Redd-Paper.pdf)

## What it contains

The ReddPaper is the project’s updated whitepaper (originally April 2014), covering:

- Proof of Work and Proof of Stake history in ReddCoin
- Proof of Stake Velocity (PoSV) design + motivations
- Monetary concepts (medium of exchange, store of value, inflation/deflation)
- ReddCoin’s position as a "digital social currency" and tipping primitive

## Table of contents (from the PDF)

- Abstract
- 1. Introduction
  - 1.1 Goals of this Redd Paper
- 2. Technology
  - 2.1 Proof of Work
  - 2.2 Proof of Stake
  - 2.3 Proof of Stake Velocity (PoSV)
- 3. Economics
  - 3.1 Medium of Exchange
  - 3.2 Unit of Account
  - 3.3 Store of Value
  - 3.4 Deflation vs Inflation
- 4. Digital Social Currency
  - 4.1 Social vs Commercial
  - 4.2 Transition from PoW to PoSV
  - 4.3 Hard to Clone
- 5. Conclusion
- 6. Five Year Retrospective
  - 6.1 Analysis, Discussion & Proposed Enhancements: “Introducing PoSV v2.0”

## Related pages in this knowledgebase

- [PoSV](../tech/consensus/posv.md)
- [PoSV v2](../tech/consensus/posv-v2.md)
